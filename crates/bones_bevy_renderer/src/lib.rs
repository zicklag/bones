    ...
    for (bevy_ent, mut tile_map, mut atlas, mut transform) in &mut bevy_bones_tile_layers {
        if let Some(bones_ent) = bones_tile_layer_entity_iter.next() {
            ...
            *transform = bones_transform.into_bevy();
            ...
            // TODO: This is probably a bug in bevy_simple_tilemap.
            // If the tilemap atlas has been changed, and one of the tiles in the map had a tile
            // index greater than the max tile count in the new atlas, the map renderer will panic.
            //
